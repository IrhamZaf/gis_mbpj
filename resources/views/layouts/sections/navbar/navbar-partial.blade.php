...
<div class="navbar-nav-right d-flex align-items-center justify-content-end" id="navbar-collapse">
  ...
  <ul class="navbar-nav flex-row align-items-center ms-md-auto">
    ...
    @auth
    @php
      $gisNavNotifications = auth()->user()->unreadNotifications()->limit(8)->get();
    @endphp
    <li class="nav-item dropdown-notifications navbar-dropdown dropdown me-2 me-xl-0">
      <a class="nav-link dropdown-toggle hide-arrow btn btn-icon btn-text-secondary rounded-pill"
        ...
        <span class="position-relative">
          ...
          @if ($gisNavNotifications->count() > 0)
          <span class="badge rounded-pill bg-danger badge-dot badge-notifications border"></span>
          @endif
        </span>
      </a>
      <ul class="dropdown-menu dropdown-menu-end dropdown-styles w-px-450 p-0">
        <li class="dropdown-menu-header border-bottom">
          <div class="dropdown-header d-flex align-items-center py-6">
            <h6 class="mb-0 me-auto">Notifikasi</h6>
            @if ($gisNavNotifications->count() > 0)
            <form action="{{ route('notifications.read-all') }}" method="POST" class="d-inline">
              @csrf
              <button type="submit" class="btn btn-sm btn-text-secondary">Tanda semua dibaca</button>
            </form>
            @endif
          </div>
        </li>
        <li class="dropdown-notifications-list scrollable-container">
          @forelse ($gisNavNotifications as $note)
          @php $payload = $note->data; @endphp
          <a href="{{ $payload['url'] ?? url('/') }}" class="dropdown-item">
            <div class="d-flex">
              ...
              <div class="flex-grow-1">
                <h6 class="small mb-0">{{ $payload['title'] ?? 'GIS' }}</h6>
                <small class="text-body-secondary">{{ $payload['message'] ?? '' }}</small>
              </div>
            </div>
          </a>
          @empty
          <div class="dropdown-item text-muted small">Tiada notifikasi baharu.</div>
          @endforelse
        </li>
      </ul>
    </li>
    ...
    <li class="nav-item navbar-dropdown dropdown-user dropdown">
      ...
      <ul class="dropdown-menu dropdown-menu-end">
        ...
        @if (class_exists(\Laravel\Jetstream\Jetstream::class) && Auth::user() && \Laravel\Jetstream\Jetstream::hasTeamFeatures())
        <li>
          <div class="dropdown-divider my-1 mx-n2"></div>
        </li>
        <li>
