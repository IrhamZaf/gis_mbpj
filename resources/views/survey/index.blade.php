...
@section('title', 'Hantaran Surveyor')
...
<div class="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-2">
  <div>
    <h4 class="mb-1">Hantaran Surveyor</h4>
    <p class="text-muted mb-0">Senarai survey dan muat naik data baharu</p>
  </div>
  <button type="button" class="btn btn-primary" data-bs-toggle="collapse" data-bs-target="#uploadFormCollapse" aria-expanded="false">
    <i class="ti tabler-upload me-1"></i>Muat naik baharu
  </button>
</div>

{{-- Collapsible upload form --}}
<div class="collapse mb-4" id="uploadFormCollapse">
  <div class="row g-4">
    <div class="col-lg-7">
      <div class="card">
...
<div class="card">
  <div class="card-header d-flex justify-content-between align-items-center flex-wrap gap-2">
    <h5 class="mb-0">Senarai hantaran survey</h5>
  </div>
    ...
    <table class="table">
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>v{{ $s->version }}</td>
          <td><span class="badge bg-label-secondary">{{ $s->reviewStatusLabel() }}</span></td>
          <td>
            {{ $s->vendor_name ?: '—' }}<br>
            <span class="small text-muted">{{ $s->surveyor_name ?: $s->surveyor?->name }}</span>
          </td>
          <td>{{ $s->survey_type ? \Illuminate\Support\Str::limit($s->survey_type, 24) : '—' }}</td>
          <td>{{ $s->incident?->incident_number ?? '—' }}</td>
          <td>{{ $s->uploads->count() }}</td>
          <td class="text-nowrap">
            @if($s->incident_id && $s->surveyor_id === auth()->id())
            <a class="btn btn-sm btn-label-primary" href="{{ route('survey.index', ['parent' => $s->id]) }}"
              onclick="document.getElementById('uploadFormCollapse')?.classList.add('show')">Versi baharu</a>
            @endif
          </td>
        </tr>
        @endforeach
